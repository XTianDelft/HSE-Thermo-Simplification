...
soil_props = {
    'sand': (2.1771, 2.9288e6),
    'clay': (1.5910, 2.9288e6)
}
...
degangle = 40
tilt = degangle/180*np.pi
boreholes_confs = {
    'comsol (medemblik) setup': setup_bh,
    f'ring of {Nb} H={Hs.mean():.0f} m tilt={degangle}°': [
    ...
    f'ring of {Nb-1} and a center bh H={Hs.mean():.0f} m tilt={degangle}°': [
...
if True:
# for soil_type, (k_s, Cp) in soil_props.items():
    alpha = 1e-6
    # alpha = k_s / Cp
    print(f'α = {alpha:.2e}')
    ...
    for conf_name in boreholes_confs:
        ...
        time_arr = gt.utilities.time_geometric(3600, 1000 * 8766 * 3600, 50)
        ...
        x_vals = np.log(time_arr/ts)
        plt.plot(x_vals, gfunc.gFunc, label=conf_name)
        # plt.semilogx(time_arr, gfunc.gFunc, label=conf_name + f' ({soil_type})', linestyle='-' if soil_type == 'sand' else '--')

plt.grid()
plt.xlim(x_vals.min(), x_vals.max())
# plt.xlim(time_arr.min(), time_arr.max())
# plt.axvline(20*8766*3600, c='k', lw=1, ls='--')
# plt.text(22*8766*3600, 4, '20 years', rotation=90)

plt.xlabel(r'ln$(t/t_s)$')
# plt.xlabel(r't (s)')
plt.ylabel(r'$g$-function')
plt.legend(loc='lower right')
plt.gcf().set_size_inches(7, 5)
plt.tight_layout()
plt.savefig("results/gfuncs1.pdf")
plt.show()
